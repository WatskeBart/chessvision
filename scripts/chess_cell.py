...
class ChessCell:
    ...
    def canMove(self, cell: ChessCell):
        ...
        return self.piece.is_valid_move(x, y, None)

    def move(self, cell: ChessCell):
    ...
    def draw_cell(self, frame: cv2.Mat):
        ...
        cv2.putText(frame, self.name, (self.x + 5, self.y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
    ...
    def has_piece(self, frame: cv2.Mat, draw: bool = False):
        ...
        if len(contours) > 0:
            ...
            piece = cv2.bitwise_and(gray, gray)
            _, thresh = cv2.threshold(gray, 0, 150, cv2.THRESH_BINARY)

            white_ratio = cv2.countNonZero(thresh) / (cell.shape[0] * cell.shape[1])
       
            if draw:
                cv2.drawContours(cell[padding:-padding, padding:-padding], [merged], -1, (0, 255, 0), cv2.FILLED)

            return (True, white_ratio > 0.75)
